...
import { Shield, Sparkles, Bell, Search, UserCheck, ChevronDown, Check, Activity, Radio, FileText } from 'lucide-react';
import { useThreat } from '../context/ThreatContext';
import { useNavigate } from 'react-router-dom';

export const Navbar = () => {
  const { currentAnalysis, presets, loadPreset, activeUser, setActiveUser } = useThreat();
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-slate-800/80 bg-slate-950/80 backdrop-blur-xl px-6 py-3 flex items-center justify-between shadow-lg shadow-black/20">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative hidden md:block">
          ...
          >
            ...
            <span className="font-semibold text-cyan-300 max-w-[200px] truncate">
              {currentAnalysis?.name || 'Select Preset'}
            </span>
                ...
                {presets.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`px-2 py-0.5 rounded text-[10px] font-mono ${p.threatLevel === 'CRITICAL' ? 'bg-rose-500/20 text-rose-400' : p.threatLevel === 'SAFE' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}>
                        {p.badge}
                      </span>
                    </button>
                  );
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-900/80 border border-slate-800 text-xs">
          ...
          <span className="font-mono text-[11px] text-slate-300">NIC/CERT-In AI Feed: Online</span>
        </div>
