 ...
 */
function getFormattedTimestampForFilename(date = new Date()) {
...
/**
 * Generate a professional forensic PDF report for PhishGuard AI
 * 
 * @param {Object} data - Analysis result data
 * @returns {Buffer} PDF buffer
 */
function generatePhishGuardPDF(data = {}) {
  ...
  const reportId = data.historyId || data.id || data.reportId || crypto.randomUUID();
  ...
  // If specific standard threat categories weren't explicitly provided, ensure key categories are evaluated
  ...
  const colors = {
    white: [255, 255, 255],
    slate900: [15, 23, 42],      // #0F172A primary headers
    slate800: [30, 41, 59],      // #1E293B
    slate700: [51, 65, 85],      // #334155
    slate600: [71, 85, 105],     // #475569
    slate400: [148, 163, 184],   // #94A3B8
    slate200: [226, 232, 240],   // #E2E8F0 borders
    slate100: [241, 245, 249],   // #F1F5F9 card bg
    slate50: [248, 250, 252],    // #F8FAFC light table zebra
    bluePrimary: [2, 132, 199],  // #0284C7 brand accent
  ...
  // Draw header top strip on subsequent pages
  function drawHeaderBannerSmall() {
    ...
    doc.text(`Report ID: ${reportId.slice(0, 18)}...`, pageWidth - marginX - 3, currentY + 4.8, { align: "right" });
  ...
  // Shield inner icon (stylized check / radar tick)
  ...
  // Left & Mid Columns: Sender, Subject, Domain, Analyzed time
  ...
  // Left Circular Risk Score Dial (Vector Graphic)
  ...
  // Draw primary arc indicator (circular approximation)
  doc.circle(dialCenterX, dialCenterY, dialRadius, "S");

  // Inner fill for gauge center
  ...
  // Blue quote line on left of summary box
  doc.setFillColor(...colors.bluePrimary);
